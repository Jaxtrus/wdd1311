...
function renderStars(rating) {
  ...
  return `
    <span class="rating" role="img" aria-label="Rating: ${rating} out of 5 stars">
      ${'⭐'.repeat(fullStars).split('').map(star => `<span aria-hidden="true" class="icon-star">${star}</span>`).join('')}
      ${'☆'.repeat(emptyStars).split('').map(star => `<span aria-hidden="true" class="icon-star-empty">${star}</span>`).join('')}
    </span>
  `;
}
